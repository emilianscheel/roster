...
export type GradientCanvasProps = {
  ...
  /** When true, fills the parent instead of forcing a full-screen shell. */
  compact?: boolean;
  /** Needed for reliable PNG/GIF capture of the WebGL buffer. */
...
export function GradientCanvas({
  ...
  useEffect(() => {
    ...
    // Use layout size (offset*), not getBoundingClientRect — CSS transforms
    // on ancestors shrink the rect and would under-allocate the GL buffer.
    const syncBufferSize = () => {
      const cssWidth = Math.max(1, canvas.offsetWidth || canvas.clientWidth);
      const cssHeight = Math.max(1, canvas.offsetHeight || canvas.clientHeight);
